...
<div class="stock-in-form">
    ...
    <div class="form-group field-stockin-price">
        <label class="control-label col-sm-3" for="stockin-price">Đơn giá</label>
        <div class="col-sm-6">
            <input type="hidden" id="stockin-price" value="<?php echo $model->price?>" class="form-control" name="StockIn[price]">
    ...
    <?= $form->field($model, 'provider')->textarea(['rows' => 2]) ?>
...
<script>
    ...
    $(document).ready(function(){
      ...
      <?php if($product_id != ''){?>
         $("#stockin-product_id").val(<?php echo $model->product_id?>).select2({
      <?php }else{ ?>
         $("#stockin-product_id").select2({
      <?php  }?> 
         lang:'vi',
         multiple: false,
