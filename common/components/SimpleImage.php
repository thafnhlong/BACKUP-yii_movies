...
class SimpleImage {
    ...
    public $media_domain = ''; //'http://media.phimbathu.com';
    ...
    function __construct($filename = null, $width = null, $height = null, $color = null) {
        ...
        if(!file_exists($filename)){
            $filename = $_SERVER['DOCUMENT_ROOT'].'/uploads/images/default.png';
        }
        //echo $filename;die;
        if ($filename) {
            $this->load($filename);
    ...
    function crop($x1, $y1, $x2, $y2) {
        ...
        // Perform crop
        $crop_width = floor($crop_width);
        $crop_height = floor($crop_height);
        $x1 = floor($x1);
        $y1 = floor($y1);
        
        //echo $crop_width.'--'.$crop_height;die;
    ...
    function resizeToWidth($width)
    {
        ...
        if ($this->width > 0)
        {
            if(array_key_exists($width,$size_default)){
                $height = $size_default[$width];
            }else{
                $ratio = $width / $this->width;
                $height = ceil($this->height * $ratio);
            }
    ...
    public function makeThumbnail($file_name, $sizes = array('380'))
    {
        ...
        {
            $source_file = $_SERVER['DOCUMENT_ROOT'] . $this->getFullPathImageByName($file_name);
            //echo $source_file;die();
            foreach ($sizes as $width)
            {
                $this->load($source_file);
                $this->resizeToWidth($width);
                $path_to_save = $_SERVER['DOCUMENT_ROOT'] . $this->getOnlyPathImageByName($file_name) .$width;
                //echo 'file:'.$file_name . ' - path:'.$path_to_save.'<br />';
    ...
    public function getThumbnail($file_name, $size = 380)
    {
        ...
        $file = $_SERVER['DOCUMENT_ROOT'] . $relative_image;
        if (file_exists($file))
        {
            
			return $this->media_domain.$relative_image;
        } else
        {
    ...
    public function upload($file,$watermarks = false){
        $max_file_size = 1 * 1024 * 1024; // Giới hạn dung lượng
        $max_width_size = 1500; // Giới hạn độ rộng 
        $new_name = '';
        $allow_types = array(
            'image/jpeg',
            'image/png',
        ...
        $path_save_image = $_SERVER['DOCUMENT_ROOT'] . $relative_path;
        ...
        if(!in_array($file['type'],$allow_types)){
            $data['error'][] = 'Định dạng file không chính xác';
        }elseif(isset($image_info[0]) && $image_info[0] > $max_width_size){
            $data['error'][] = 'Độ rộng vượt quá 1500px';
        }else{
            $tmp_name = CVietnameseTools::makeUrlFriendly($file['name']);
            $exploded = explode('.', $tmp_name);
            $new_name = $exploded[0] . '-' . date('Ym') . rand(111, 999) . '.' . $exploded[1];
            if (move_uploaded_file($file['tmp_name'], $path_save_image . $new_name)) {
                if ($watermarks) {
    ...
    public static function addWaterMarks($image, $compression = 90)
    {
        ...
        $wmsource = Yii::getAlias('@@webroot') . '/images/watermarks.png';
